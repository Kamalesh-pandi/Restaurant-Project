...
export default function MenuCatalogManager({ showToast }) {
  ...
  const filteredItems = menuItems.filter((item) => {
    ...
    let matchesTab = true;
    if (filterTab === 'SPECIALS') matchesTab = Boolean(item.isSpecial);
    else if (filterTab === 'COMBOS') matchesTab = Boolean(item.isCombo);
    else if (filterTab === 'OUT_OF_STOCK') matchesTab = item.isAvailable === false;
    else if (filterTab === 'VEG') matchesTab = item.isVeg !== false;
    else if (filterTab === 'NON_VEG') matchesTab = item.isVeg === false;
                  ...
                  <div key={itemId} className="col-12 col-md-6 col-xl-4">
                    <div className={`card bg-surface border-custom shadow-sm h-100 overflow-hidden position-relative ${isOut ? 'opacity-75' : ''}`}>
                      ...
                      <div className="position-absolute top-0 start-0 m-2 d-flex flex-column gap-1" style={{ zIndex: 5 }}>
                        <span className={`badge ${item.isVeg ? 'bg-success text-white' : 'bg-danger text-white'}`}>

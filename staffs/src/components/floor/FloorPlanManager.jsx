...
export default function FloorPlanManager({ showToast }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [tblsRes, waitRes, outsRes, ordersRes, turnRes, billsRes] = await Promise.all([
      ...
      const bls  = Array.isArray(billsRes) ? billsRes : [];

      const billMap = {};
      bls.forEach(b => { if (b.orderId) billMap[b.orderId] = b; });

      const enrichedOrders = ords.map(o => {
        const oId = o.orderId || o.id;
        const b = billMap[oId];
        const amt = b?.total != null ? Number(b.total) : (o.totalAmount != null ? Number(o.totalAmount) : 0);
        return { ...o, totalAmount: amt, total: amt };
      });

            ...
            <tbody>
              {promotions.length === 0 ? (
                ...
                promotions.map((p, idx) => (
                  <tr key={p.id || idx}>
                    <td><code className="fw-bold text-danger fs-6">{p.code}</code></td>
                    <td className="fw-bold text-main">{p.title}</td>
                    <td className="fw-bold text-success">{p.discountPercent}% OFF</td>
                    <td className="text-muted-custom">₹{p.minBillValue || 0}</td>
                    <td className="text-muted-custom small">{p.validUntil || 'Ongoing'}</td>
